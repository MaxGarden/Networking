...
#if defined(_WIN32)
#include "webby/webby_win32.h"
#else
#include "webby/webby_unix.h"
#endif
...
struct WebbyServer*
WebbyServerInit(const struct WebbyServerConfig *config, void *memory, size_t memory_size)
{
  int i;
  ...
  int set = 1;
  setsockopt(server->socket, SOL_SOCKET, SO_NOSIGPIPE, (void*)&set, sizeof(int));
#endif
...
static void reset_connection(struct WebbyServer *srv, struct WebbyConnectionPrv *conn, int newConnection)
{
  ...
  if (newConnection)
  {
      ...
      memset(conn->public_data.address, 0, sizeof(conn->public_data.address));
  }
...
static int wb_on_incoming(struct WebbyServer *srv)
{
  ...
  webby_socklen_t client_addr_len = sizeof clientAddress;
  webby_socket_t fd;
  ...
  fd = accept(srv->socket, (struct sockaddr*) &clientAddress, &client_addr_len);
  ...
  memcpy(connection->public_data.address, inet_ntoa(clientAddress.sin_addr), sizeof(connection->public_data.address));
...
static int scan_websocket_frame(const struct WebbyBuffer *buf, struct WebbyWsFrame *frame, int offset)
{
  unsigned char flags = 0;
  unsigned int len = 0;
  unsigned int opcode = 0;
  unsigned char* data = buf->data + offset;
  unsigned char* data_max = data + buf->used - offset;
  int i;
  int len_bytes = 0;
  int mask_bytes = 0;
  ...
  frame->header_size = (unsigned char)(data - (buf->data + offset));
  frame->flags = flags;
  ...
  for (;;)
  {
    switch (connection->state)
    {
      ...
      case WBC_WEBSOCKET: {
        ...
        int offset = 0;
        while (offset < connection->io_buf.used || connection->io_buf.used == 0)
        {
            if (0 != scan_websocket_frame(&connection->io_buf, &connection->ws_frame, offset))
            {
                /* Nothing yet */
                return;
            }
            ...
            connection->io_data_left = connection->io_buf.used - connection->ws_frame.header_size - offset;
            ...
            offset += connection->ws_frame.payload_length + connection->ws_frame.header_size;
        }
...
struct WebbyConnection*
WebbyFindConnectionFromUserData(struct WebbyServer *srv, const void* userData)
{
    int i;
    int count;

    for (i = 0, count = srv->connection_count; i < count; ++i)
        if (srv->connections[i].public_data.user_data == userData)
            return &srv->connections[i].public_data;
